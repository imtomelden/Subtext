...
enum MDXSerialiser {
    ...
    private static func renderFrontmatter(_ f: ProjectFrontmatter) -> String {
        ...
        if let challenge = f.challenge { lines.append(keyValue("challenge", quoted: challenge)) }
        if let approach = f.approach { lines.append(keyValue("approach", quoted: approach)) }
        if let outcome = f.outcome { lines.append(keyValue("outcome", quoted: outcome)) }
        ...
        if let headerImage = f.headerImage, !headerImage.isEmpty { lines.append(keyValue("headerImage", raw: headerImage)) }
        if let ext = f.externalUrl, !ext.isEmpty { lines.append(keyValue("externalUrl", raw: ext)) }
        if f.draft { lines.append("draft: true") }

        if let hero = f.hero, !hero.isEmpty {
            lines.append("hero:")
        ...
        if let videoMeta = f.videoMeta, !videoMeta.isEmpty {
    ...
    private static func renderBlock(_ block: ProjectBlock) -> [String] {
        ...
        case .keyStats(let b):
            ...
            for item in b.items {
                ...
                l.append("        \(keyValue("lastUpdated", raw: item.lastUpdated))")
            }
            return l
